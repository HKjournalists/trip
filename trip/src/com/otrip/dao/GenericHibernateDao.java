...
public abstract class GenericHibernateDao<T extends Serializable, PK extends Serializable> extends HibernateDaoSupport implements IGenericDao<T, PK> {

	static Logger log = Logger.getLogger(GenericHibernateDao.class);
   ...
   //@Qualifier("sessionFactory")
	...
	public List queryHql(final String hql){
		return getSession().createQuery(hql).list();
	}
	
	@SuppressWarnings("unchecked")
	public void batchSaveEntity(final List<T> ts) {   
	...
	@SuppressWarnings("unchecked")
	public List<T> findByInCondition(final String hql,String inParamName,Object [] inParams){
		return getSession().createQuery(hql).setParameterList(inParamName, inParams).list();
	}
	...
	public List findBySQL(String sql,String[] params)
	{
		Query query=getHibernateTemplate().getSessionFactory().getCurrentSession().createSQLQuery(sql).setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP);
		if(params.length!=0)
		{
	 ...
	 */
	@Override
	public List findLimitNum(String hql,int num,int start) {
	...
	@Override
	public List findLimitNumByParam(String hql,int num,int start,Object[] objects)
	{
		Query query= this.sessionFactory.getCurrentSession().createQuery(hql);
		if(objects.length!=0)
		{
